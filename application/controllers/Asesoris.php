...
class Asesoris extends CI_Controller {

  function __construct(){
    parent:: __construct();
    $this->load->model('M_asesoris','m'); //load model, simpan ke m
    $this->load->model('M_model','mm'); //load model pemakai, simpan ke mk
    $this->load->model('M_pemakai','mp'); //load model pemakai, simpan ke mk
  }

	function index(){
    $data['d_asesoris']  = $this->m->ambilData(); //jalankan fungsi ambilData, simpan ke $data
    $data['d_model']  = $this->mm->ambilDatabyTipe('2'); //Asesoris
    $data['d_pemakai']=$this->mp->ambilDataIDdanNama();
    //$data['d_manufaktur']  = $this->mm->ambilData();

    $this->load->view('header');
		$this->load->view('leftside');
		$this->load->view('asesoris/index', $data); //load index model, bypass $data
		$this->load->view('footer_js');
    $this->load->view('asesoris/ajax_scripts', $data);
  ...
  function tampilcheck($id){
    //mengambil model ID dari id asesoris
    ...
    $data['d_asesoris']=$this->m->ambilDatabyID($id);
  ...
  function allcheckin(){
    ...
    $this->load->view('header');
		$this->load->view('leftside');
		$this->load->view('asesoris/allcheckin', $data); //load index model, bypass $data
		$this->load->view('footer_js');
    $this->load->view('asesoris/ajax_scripts', $data);
    $this->load->view('footer');
	}
  ...
  function allcheckout(){
    ...
    $this->load->view('header');
		$this->load->view('leftside');
		$this->load->view('asesoris/allcheckout', $data); //load index model, bypass $data
		$this->load->view('footer_js');
    $this->load->view('asesoris/ajax_scripts', $data);
    $this->load->view('footer');
	}
