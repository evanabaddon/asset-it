...
class Model extends CI_Controller {
  ...
  //$this->load->model('m_fungsi','mf');
    $this->load->model('m_kategori','mk'); //load model kategori, simpan ke mk
    $this->load->model('m_manufaktur','mm'); //load model kategori, simpan ke mm
  }
    ...
    $data['d_model']  = $this->m->ambilDataByLimit($config['per_page'], $page); //jalankan fungsi ambilData, by pagination
    ...
    $this->load->view('header');
		...
		$this->load->view('model/index', $data); //load index kategori, bypass $data
		$this->load->view('footer');
	}


  function tambah(){
    $data['d_model']  = $this->m->ambilData(); //jalankan fungsi ambilDataTipe, simpan ke $data
    $data['d_kategori']  = $this->mk->ambilData();
    $data['d_manufaktur']  = $this->mm->ambilData();

    $this->load->view('header');
    $this->load->view('leftside');
    $this->load->view('model/tambah', $data);
    $this->load->view('footer');
  }
  ...
  function submit(){
    ...
    redirect(base_url('model/index'));
  }

  function ubah ($id){
    $data['d_model']  = $this->m->ambilDataID($id); //jalankan fungsi ambilData berdasarkan ID, simpan ke $data
    $data['d_manufaktur']  = $this->mm->ambilData(); //jalankan fungsi ambilData berdasarkan ID, simpan ke $data
    $data['d_kategori']  = $this->mk->ambilData(); //jalankan fungsi ambilDataTipe, simpan ke $data

    $this->load->view('header');
    $this->load->view('leftside');
    $this->load->view('model/ubah', $data);
    $this->load->view('footer');
